...
export async function getEquipmentCoverage(equipmentId) {
  const [warranties, amc, parts] = await Promise.all([
  ...
  if (warranties.error || amc.error || parts.error) throw warranties.error || amc.error || parts.error
  const warrantyHistory = warranties.data ?? []
  return { warranty: warrantyHistory.find((item) => item.status === 'Active') ?? null, historicalWarranty: warrantyHistory[0] ?? null, amc: amc.data?.[0] ?? null, partWarranties: parts.data ?? [] }
}
