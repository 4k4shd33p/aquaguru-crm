...
function paymentHistoryRows(payments = []) {
  ...
  return payments.map((payment) => ({ ...payment, correction_of_payment: payment.correction_of_payment_id ? byId.get(payment.correction_of_payment_id) ?? null : null, corrected_by_payment: payment.corrected_by_payment_id ? byId.get(payment.corrected_by_payment_id) ?? null : null }))
}
...
export async function getService(serviceId) {
  ...
  const service = { ...data, service_payments: paymentHistoryRows(data.service_payments) }
  return { ...service, totals: serviceTotals(service), financials: calculateServiceFinancials(service), components: components.data ?? [], newPartWarranties: warranties.data ?? [] }
}

export async function getServiceEconomicsBreakdown(serviceId) {
  const { data, error } = await client().rpc('get_service_economics_breakdown', { p_service_id: serviceId })
  if (error) throw error
  return data?.[0] ?? null
}
