...
export async function getServiceLookups() {
  const [parts, serviceTypes, technicians, paymentMethods] = await Promise.all([
    ...
    client().from('service_types').select('id, name').eq('is_active', true).order('name'),
