...
function economicTarget(item) {
  if (item.coverage_type === 'AMC') return `AMC:${item.amc_cycle_id || ''}`
  if (item.coverage_type === 'Equipment Warranty') return `Equipment Warranty:${item.equipment_warranty_id || ''}`
  if (item.coverage_type === 'Part Warranty') return `Part Warranty:${item.service_item_warranty_id || ''}`
  return item.coverage_type || 'Other'
}
...
export function StructuralServiceItemCorrection({ service, item, parts, coverage, allocations, onBack, onSaved }) {
  ...
  const contexts = new Set(allocationItems.filter((source) => source.coverage_type !== 'Other').map(economicTarget))
  const sharedCostTotal = number(service.technician_charge) + number(service.travel_cost) + number(service.other_direct_cost)
  const economicTargetChanged = economicTarget(item) !== economicTarget({ ...item, ...draft })
  const showAllocations = contexts.size > 1 && sharedCostTotal > 0 && economicTargetChanged
