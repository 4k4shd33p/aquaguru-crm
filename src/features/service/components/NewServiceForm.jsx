...
function ReviewSummary({ values, equipment, items, financials }) {
  ...
  return <div className="service-review">
    <section><h3>Service details</h3><dl className="service-review__facts"><div><dt>Service Date</dt><dd>{values.serviceDate || 'Not set'}</dd></div><div><dt>Equipment</dt><dd>{equipment?.equipment_code || 'Not set'}</dd></div><div><dt>Service Type</dt><dd>{values.serviceTypeId ? 'Selected service type' : 'Not set'}</dd></div><div><dt>Status</dt><dd>{values.status}</dd></div><div><dt>Technician</dt><dd>{values.technicianId ? 'Assigned' : 'Not assigned'}</dd></div><div><dt>Recommended Next Service</dt><dd>{values.nextServiceDue || 'Not set'}</dd></div></dl>
...
export function NewServiceForm({ initialEquipment, onSubmit, onCancel, isSubmitting }) {
  ...
  return <form className="service-wizard" onSubmit={handleSubmit(submit)}>
    ...
    {step === 4 && <section className="service-wizard__panel"><h3>Review service</h3><p className="form-help">Review is read-only until you explicitly complete this Service below.</p><ReviewSummary values={values} equipment={equipment} items={items} financials={financials} /><label>Notes<textarea {...register('notes')} /></label></section>}
