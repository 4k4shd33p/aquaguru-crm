...
export function CoverageDirectory({ kind, rows, count, page, onPage, isLoading, emptyTitle, emptyDescription }) {
  ...
  return <Card className="coverage-directory"><div className="coverage-table"><div className="coverage-table__head"><span>{isAmc ? 'AMC' : isPart ? 'Part warranty' : 'Warranty'}</span><span>Equipment / customer</span><span>Dates</span><span>{isAmc ? 'Totals / visits' : isPart ? 'Part' : 'Visits'}</span><span>Status</span></div>{rows.map((row) => { const link = isAmc ? `/coverage/amc/${row.id}` : isPart ? `/coverage/part-warranties/${row.id}` : `/coverage/warranties/${row.id}`; const code = row.amc_code ?? row.part_warranty_code ?? row.warranty_code; const equipment = row.equipment; return <Link key={row.id} className="coverage-table__row" to={link}><strong>{code}<ExternalLink size={14} /></strong><div><b>{equipmentLabel(equipment)}</b><small>{equipment?.customers?.name ?? 'Customer unavailable'} · {equipment?.product_models?.model_name ?? 'Model not recorded'}</small></div><div><b>{date(row.start_date)}</b><small>to {date(row.end_date)}</small></div><div>{isAmc ? <><b>{row.totals?.agreed === null ? 'Charge unavailable' : money(row.totals?.agreed)}</b><small>{money(row.totals?.collected)} collected · {(row.services ?? []).length} visits</small></> : isPart ? <><b>{row.parts?.name ?? 'Part unavailable'}</b><small>{row.duration_months} months</small></> : <><b>{row.planned_visits} planned</b><small>{(row.services ?? []).length} actual visits</small></>}</div><Badge tone={statusTone(row.status)}>{row.status}</Badge></Link> })}</div><Pager page={page} count={count} onPage={onPage} /></Card>
}
