...
export function FinanceSummary({ summary, asOf }) {
  return <>
    ...
    <section className="finance-breakdown" aria-label="Category performance">
      <CategoryCard title="Sales" value={summary?.sales_value} valueAvailable={summary?.total_transaction_value_available} collections={summary?.sales_collections} outstanding={summary?.sales_outstanding_as_of} cost={summary?.sales_known_direct_cost} costAvailable={summary?.sales_direct_cost_available} contribution={summary?.sales_contribution} contributionAvailable={summary?.sales_contribution_available} extras={<><span>Direct collections: {formatMoney(summary?.sales_direct_collections)}</span><span>EMI collections: {formatMoney(summary?.sales_emi_collections)}</span></>} />
      <CategoryCard title="Service" value={summary?.service_value} valueAvailable={summary?.total_transaction_value_available} collections={summary?.service_collections} outstanding={summary?.service_outstanding_as_of} cost={summary?.service_known_direct_cost} costAvailable={summary?.service_direct_cost_available} contribution={summary?.service_contribution} contributionAvailable={summary?.service_contribution_available} />
