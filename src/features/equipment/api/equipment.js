...
export async function getEquipmentCustomerLocations(customerId) { if (!customerId) return []; const { data, error } = await requireClient().from('locations').select('id, location_code, location_name, area, city, pincode').eq('customer_id', customerId).eq('is_active', true).order('created_at'); if (error) throw error; return data ?? [] }
export async function getCustomerEquipment(customerId) { const { data, error } = await requireClient().from('equipment').select(equipmentFields).eq('customer_id', customerId).order('created_at', { ascending: false }).limit(50); if (error) throw error; return data ?? [] }
export async function getEquipmentComponents(equipmentId) { const { data, error } = await requireClient().from('equipment_components').select(componentFields).eq('equipment_id', equipmentId).order('removed_date', { ascending: true, nullsFirst: true }).order('installed_date', { ascending: false, nullsFirst: false }).order('created_at', { ascending: false }); if (error) throw error; return (data ?? []).filter((component) => component.parts?.equipment_tracking_enabled) }
...
export async function decommissionEquipment(equipmentId) { const { data, error } = await requireClient().from('equipment').update({ status: 'Decommissioned' }).eq('id', equipmentId).select(equipmentFields).single(); if (error) throw error; return data }
