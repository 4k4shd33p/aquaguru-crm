import { Boxes, CalendarClock, CircleDollarSign, Plus, Wrench } from 'lucide-react'
...
function coverageSummary(summary) {
  if (summary?.amc) return `AMC ${summary.amc.amc_code} · Active through ${formatDate(summary.amc.end_date)}`
...
export function CustomerEquipmentSection({ customer, equipment, isLoading }) {
  ...
  })}</div> : <div className="customer-equipment__empty"><Boxes size={20} /><span>This customer has no equipment recorded.</span></div>}<p className="customer-equipment__finance"><CircleDollarSign size={16} />Customer-level Amount Due is not shown here because the current reporting API is paginated and does not accept a customer filter. This avoids an incomplete or double-counted total.</p></Card>
}
