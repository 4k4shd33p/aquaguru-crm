...
export function EquipmentLocationHistory({ equipmentId }) {
  ...
  return (
    <Card className="equipment-location-history">
      ...
      {movements.length ? (
        <div className="location-history-list">
          {movements.map((movement) => {
            const source = sourceLabel(movement)
            return (
              <article className="location-history-item" key={movement.id}>
                ...
                {(source || movement.notes) && <div className="location-history-item__details">{source && <small>{source}</small>}{movement.notes && <small>{movement.notes}</small>}</div>}
              </article>
            )
          })}
        </div>
      ) : <p className="component-empty">No known location moves have been recorded.</p>}
    </Card>
  )
}
