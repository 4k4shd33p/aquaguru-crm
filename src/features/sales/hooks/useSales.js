...
function mutation(mutationFn) { const queryClient = useQueryClient(); return useMutation({ mutationFn, onSuccess: (_, variables) => { queryClient.invalidateQueries({ queryKey: saleKeys.all }); if (variables?.saleId) queryClient.invalidateQueries({ queryKey: saleKeys.detail(variables.saleId) }) } }) }
