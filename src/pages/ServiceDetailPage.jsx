...
const componentLabel = (type) => ({ Item: 'Work-item direct cost', Technician: 'Technician Cost', Travel: 'Travel / Petrol Cost', Other: 'Other Direct Cost' }[type] || type)
...
function CostBreakdown({ economics, financials, service }) {
  ...
  return <section className="card"><h2>Cost breakdown</h2>{isPurePaid && <div className="service-financial-detail"><p><span>Customer Charge / Revenue</span><strong>{formatCurrency(revenue)}</strong></p><p><span>Known Direct Cost</span><strong>{formatCurrency(knownCost)}</strong></p>{!hasUnknownCost && <p><span>Gross Profit</span><strong>{formatCurrency(revenue - knownCost)}</strong></p>}</div>}{contexts.map((context) => <article className="service-detail-item" key={context.economic_bucket}><div><strong>{bucketLabel(context.economic_bucket)} {context.economic_bucket === 'AMC' ? 'Cost Incurred' : context.economic_bucket === 'Product Sales' ? 'Cost Incurred' : 'Cost'}</strong><strong>{formatCurrency(context.known_direct_cost)}</strong></div>{context.economic_bucket === 'AMC' && amcCycles.map((cycle) => <p className="service-amc-trace" key={cycle.id}><Link to={`/coverage/amc/${cycle.id}`}>{cycle.amc_code} · View AMC</Link></p>)}{(context.components || []).map((component) => <p key={component.cost_type}><span>{componentLabel(component.cost_type)}</span><strong>{Number(component.unknown_cost_count) > 0 ? 'Not recorded' : formatCurrency(component.known_direct_cost)}</strong></p>)}</article>)}<div className="service-financial-detail"><p><span>Known Direct Cost</span><strong>{formatCurrency(knownCost)}</strong></p></div>{hasUnknownCost && <p className="form-help">Some direct costs are unknown, so profitability is incomplete.</p>}</section>
}
