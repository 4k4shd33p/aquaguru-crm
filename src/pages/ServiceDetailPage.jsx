...
function FinancialDetails({ financials, context = 'None' }) {
  ...
  return <section className="card"><h2>Financial summary</h2><div className="service-financial-detail"><p><span>Usual Service Value</span><strong>{formatCurrency(financials.usualServiceValue)}</strong></p><p><span>Chargeable Subtotal</span><strong>{formatCurrency(financials.chargeableSubtotal)}</strong><small>Customer-payable work before final charge adjustments.</small></p><p><span>Final Customer Charge</span><strong>{money(financials.finalCustomerCharge)}</strong></p>{financials.chargeKnown && <><p><span>Discount</span><strong>{formatCurrency(financials.discount)}</strong></p>{financials.additionalCharge > 0 && <p><span>Additional Charge</span><strong>{formatCurrency(financials.additionalCharge)}</strong></p>}<p><span>{costLabel}</span><strong>{formatCurrency(financials.directCost)}</strong></p>{!covered && !mixed && <p><span>Profit</span><strong>{formatCurrency(financials.profit)}</strong></p>}</>}<p><span>Amount Received</span><strong>{formatCurrency(financials.amountReceived)}</strong></p><p><span>Amount Due</span><strong>{money(financials.amountDue)}</strong></p></div>{covered && <p className="form-help">{context === 'AMC' ? 'This visit fulfils an AMC contract; its cost is accounted against the AMC, not as standalone Service profit.' : 'This visit fulfils equipment warranty coverage; its cost is accounted against warranty attribution, not as standalone Service profit.'}</p>}{mixed && <p className="form-help">This Service has mixed coverage. Customer charge and costs are allocated by individual work-item coverage rather than shown as one standalone Service profit.</p>}</section>
}
...
function ServiceCorrectionForm({ service, lookups, saving, onSave, onClose }) {
