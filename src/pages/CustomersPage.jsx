...
import { useCustomers, useCreateCustomer, useCustomerTypes } from '../features/customers/hooks/useCustomers'
...
export function CustomersPage() {
  ...
  const [filters, setFilters] = useState({ page: 1, pageSize: 25, search: '', customerTypeId: '', status: '' })
  ...
  return <div className="customers-page"><div className="page-heading"><div><span className="eyebrow">Customers</span><h2>Customer directory</h2><p>Manage the people and organizations Aquaguru serves.</p></div><span className="page-heading__count"><UsersRound size={17} />{customersQuery.data?.count ?? 0} customers</span></div>{customersQuery.isError ? <ErrorState title="Customers could not be loaded" description="Please refresh and try again." /> : <CustomerList customers={customersQuery.data?.customers ?? []} total={customersQuery.data?.count ?? 0} isLoading={customersQuery.isLoading || customersQuery.isFetching && !customersQuery.data} filters={{ ...filters, customerTypes: typesQuery.data ?? [] }} onFilterChange={(changes) => setFilters((current) => ({ ...current, ...changes }))} onPageChange={(page) => setFilters((current) => ({ ...current, page }))} onNewCustomer={() => { setFormError(''); setIsCreateOpen(true) }} />}{isCreateOpen && <Modal title="New customer" description="Customer codes are generated automatically." onClose={() => setIsCreateOpen(false)}>{formError && <p className="form-message" role="alert">{formError}</p>}<CustomerForm onCancel={() => setIsCreateOpen(false)} onSave={saveCustomer} isSaving={createCustomer.isPending} /></Modal>}</div>
}
