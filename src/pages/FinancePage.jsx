...
import { useFinanceOutstanding, useFinanceRecentCollections, useFinanceSummary } from '../features/finance/hooks/useFinance'
...
export function FinancePage() {
  ...
  return <div className="finance-page"><header className="page-heading"><div><span className="eyebrow">Finance</span><h2>Finance reporting</h2><p>Read-only transaction, collection and outstanding reporting.</p></div></header><FinanceDateControls preset={preset} dates={dates} onPresetChange={selectPreset} onDateChange={setDate} />{!datesAreValid ? <div className="form-message" role="alert">End date cannot be earlier than start date.</div> : <>{summary.isError || recent.isError || outstanding.isError ? <ErrorState title="Finance data could not be loaded" description={humanError()} /> : <><FinanceSummary summary={summary.data} asOf={formatBusinessDate(dates.dateTo)} /><FinanceRecentCollections items={recent.data ?? []} isLoading={recent.isLoading} limit={recentLimit} onLimitChange={setRecentLimit} /><FinanceOutstanding result={outstanding.data} isLoading={outstanding.isLoading} category={outstandingCategory} onCategoryChange={setCategory} page={outstandingPage} onPageChange={setOutstandingPage} asOf={formatBusinessDate(dates.dateTo)} /></>}</>}</div>
}
