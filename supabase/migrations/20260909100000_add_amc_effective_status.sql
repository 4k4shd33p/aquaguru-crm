-- Effective AMC status and ordinary-creation default.
-- Stored historical statuses remain unchanged; expiry is derived at read time.

create or replace function private.effective_amc_status(
  p_status text,
  p_end_date date
)
returns text
...
revoke all on function private.effective_amc_status(text, date) from public;
revoke all on function private.effective_amc_status(text, date) from anon;
grant execute on function private.effective_amc_status(text, date) to authenticated;

create or replace view public.amc_cycles_effective
with (security_invoker = true)
as
select
  ac.*,
  private.effective_amc_status(ac.status, ac.end_date) as effective_status
from public.amc_cycles as ac;

revoke all on table public.amc_cycles_effective from public;
revoke all on table public.amc_cycles_effective from anon;
grant select on table public.amc_cycles_effective to authenticated;

-- Correct PL/pgSQL output-column ambiguity in atomic AMC creation.
...
begin
  ...
  if v_status is null then
    v_status := case when p_end_date < current_date then 'Expired' else 'Active' end;
  elsif v_status not in ('Active', 'Expired', 'Cancelled', 'Void', 'Unknown') then
  ...
  insert into public.amc_cycles as ac (
    submission_key,
  ...
  returning ac.id, ac.amc_code, ac.cycle_number
