-- Enforce AMC start dates against the same equipment's completed Initial Installation.
-- Unknown or non-completed installation history intentionally does not block AMC creation.
...
declare
  ...
  v_initial_installation_date date;
begin
  ...
  select i.installation_date
    into v_initial_installation_date
    from public.installations i
    where i.equipment_id = p_equipment_id
      and i.installation_classification = 'Initial Installation'
      and i.status = 'Completed';

  if found and p_start_date < v_initial_installation_date then
    raise exception using
      errcode = '22023',
      message = format(
        'AMC cannot start before this equipment''s initial installation date (%s).',
        to_char(v_initial_installation_date, 'DD Mon YYYY')
      );
  end if;
